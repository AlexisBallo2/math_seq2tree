   ...
   # num_total_vars = len(problem_vars[0])
    ...
    if useSNIMask:
        ...
        for i, batch in enumerate(num_window):
            ...
            for j, num in enumerate(batch):
                is_sni = models['sni'](num, problem_output[i])
    ...
    for i, num_size in enumerate(num_size_batch):
        if useCustom and useSeperateVars:
            ...
            #     if inTraining:
            #         num_mask.append([0] * output_lang.num_start + problem_vars[i].tolist() + [0] * len(generate_nums) + flipped + [1] * (max_num_size - d))
            #     else:
            ...
            #         num_mask.append([0] * output_lang.num_start + problem_var_list + [0] * len(generate_nums) + flipped + [1] * (max_num_size - d))
            # else:
...
def evaluate_tree( input_batch, input_length, target_batch, target_length, nums_stack_batch, nums_batch, num_size_batch, output_var_batches, generate_nums, models, output_lang, num_pos, equation_targets, var_pos, batch_sni, pair_mapping, solutions, useCustom, all_vars,  setName, useSemanticAlignment, useSeperateVars, useOpScaling, useSNIMask, useFixT, datasets, beam_size, inTraining, english=False):
    ...
    print()
    if setName == 'MATH' or useCustom == False:
        for i in range(num_x):
            equation = []
            try:
        ...
        for i in range(num_x):
            ...
            for j in final_beams[i][0].out:
                ...
                equation.append(output_lang.index2word[j])
            replace = replace_nums(pair_mapping[0], equation, nums_batch[0], nums_stack_batch[0])
            updated = from_prefix_to_infix(replace) 
            if updated is not None:
                    equation_set.append("".join(updated) + " = 0 " )#+ replaced_targs[each_equation])
            else:
